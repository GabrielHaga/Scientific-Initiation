...
transdutor.foco = [0e-3; 0; 0e-3]; %posição focal [m]
% Da esfera
bolinha.rho = 30; %Densidade da bolinha [kg/m^3]
bolinha.R = 1.35e-3; %Raio da esfera [m]
pos_i = [0.97278e-3; 0; 0e-3 ]; %Posição inicial da bolinha [m]
...
while i <= n
    ...
    v = [vx; vy; vz];
%     Fd = [0;0;0];
    Fgxg = -(Calc_Gorkov_estacionario(x+disc/2, y, z, transdutor) - Calc_Gorkov_estacionario(x-disc/2, y, z, transdutor))/disc;
    Fgyg = -(Calc_Gorkov_estacionario(x, y+disc/2, z, transdutor) - Calc_Gorkov_estacionario(x, y-disc/2, z, transdutor))/disc;
    Fgzg = -(Calc_Gorkov_estacionario(x, y, z+disc/2, transdutor) - Calc_Gorkov_estacionario(x, y, z-disc/2, transdutor))/disc;
    bolinha.Fgxg(i) = Fgxg;
    bolinha.Fgyg(i) = Fgyg;
    bolinha.Fgzg(i) = Fgzg;
    [F] = Acoustokinetics(x, y, z, transdutor);
...
grid on;
xlabel('Eixo z [mm]');
ylabel('Força no eixo z [\muN]');
legend('Acústica cinética', 'Gor''kov');
saveas(3,'D:\Usuarios\IC\Relatório final FAPESP\Imagens\2_Grafico_Fz_z.png');